...
if strcmp(method,'canny')
    ...
    [dx, dy] = smoothGradient(a, sigma);%figure; imshow(a,[])
    
    % Calculate Magnitude of Gradient
    magGrad = hypot(dx, dy);%figure; imshow(magGrad,[])%figure; imhist(magGrad)
    magmax = max(magGrad(:));
    if magmax > 0
        magGrad = magGrad / magmax;
    end
%     counts = imhist( magGrad,64);
%     Thresh = find(cumsum(counts) > 0.9*m*n,... %figure; imshow(magGrad>Thresh)
%         1,'first') / 64;
    lthresh  = graythresh(magGrad);%figure; imshow(magGrad>lthresh)%figure; imhist(magGrad(magGrad>lthresh))
    lthresh2 = lthresh*0.5;
    Hthresh  = graythresh(magGrad(magGrad>lthresh));
...
%     
    ...
    % Strength
    e = thinAndThreshold(dx, dy, magGrad, lthresh, lthresh);%figure; imshow(e,[])
    e2= thinAndThreshold(dx, dy, magGrad, lthresh2, lthresh2);%figure; imshow(e2,[])
    thresh = [lthresh lthresh];
...
elseif any(strcmp(method, {'log','zerocross'}))
    ...
    b = imfilter(a,op,'replicate');%figure; imshow(b,[]);
    b2 = mat2gray(b);
    gy = imfilter(b2,[-1;1],'symmetric'); %figure; imshow(abs(gy),[])
    absgy = abs(gy);
    thresh = graythresh(absgy);
    ...
    [rx,cx] = find( b(rr,cc) < 0 & b(rr,cc+1) > 0 ...
        & abs( b2(rr,cc)-b2(rr,cc+1) ) > thresh );   % [- +] %figure; imshow(b(rr,cc) < 0 & b(rr,cc+1) > 0  & abs( b(rr,cc)-b(rr,cc+1) ) > 0.001)
    e((rx+1) + cx*m) = 1;
    [rx,cx] = find( b(rr,cc-1) > 0 & b(rr,cc) < 0 ...
        & abs( b2(rr,cc-1)-b2(rr,cc) ) > thresh );   % [+ -]
    e((rx+1) + cx*m) = 1;
    [rx,cx] = find( b(rr,cc) < 0 & b(rr+1,cc) > 0 ...
        & abs( b2(rr,cc)-b2(rr+1,cc) ) > thresh);   % [- +]'
    e((rx+1) + cx*m) = 1;
    [rx,cx] = find( b(rr-1,cc) > 0 & b(rr,cc) < 0 ...
        & abs( b2(rr-1,cc)-b2(rr,cc) ) > thresh);   % [+ -]'
    e((rx+1) + cx*m) = 1;
    
    % Most likely this covers all of the cases.   Just check to see if there
    % are any points where the LoG was precisely zero:
    [rz,cz] = find( b(rr,cc)==0 );%figure; imshow(e,[])
    if ~isempty(rz)
        ...
        zz = (b(zero-1) < 0 & b(zero+1) > 0 ...
            & abs( b2(zero-1)-b2(zero+1) ) > 2*thresh);     % [- 0 +]'
        e(zero(zz)) = 1;
        zz = (b(zero-1) > 0 & b(zero+1) < 0 ...
            & abs( b2(zero-1)-b2(zero+1) ) > 2*thresh);     % [+ 0 -]'
        e(zero(zz)) = 1;
        zz = (b(zero-m) < 0 & b(zero+m) > 0 ...
            & abs( b2(zero-m)-b2(zero+m) ) > 2*thresh);     % [- 0 +]
        e(zero(zz)) = 1;
        zz = (b(zero-m) > 0 & b(zero+m) < 0 ...
            & abs( b2(zero-m)-b2(zero+m) ) > 2*thresh);     % [+ 0 -]
        e(zero(zz)) = 1;
...
E = cannyFindLocalMaxima(dx,dy,magGrad,lowThresh);%figure; imshow(E,[])
